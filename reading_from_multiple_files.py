...
if __name__ == "__main__":
    ...
    parser.add_argument("--path", default="src/sample", help="File or directory path in the GitHub repository")
    ...
    try:
        if args.github:
            code_content = get_github_file_content(args.owner, args.repo, args.path, args.branch)
            if code_content:
                ...
                print(f"Summary of {args.path} from {args.owner}/{args.repo}:")
                print("\n\n".join(summaries))
            else:
                print("Failed to fetch the file from GitHub")

            # Uncomment this block if you want to list all files in a GitHub directory.
            # files = get_github_files_in_directory(args.owner, args.repo, args.path, args.branch)
